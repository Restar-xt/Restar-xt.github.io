...
<head>
  <meta charset="UTF-8" />
  <title>PAL EFL vs Year Interactive</title>
...
<body>
  ...
  // ============================================================
  // 1) 文献数据（示例 4 篇）
  //    ❗注意：不要写 None（这版会自动兜底，但仍建议写 null 或 "None"）
  // ============================================================
  let palData = [
    {
      Year: 2007,
    ...
    },
    {
      ...
      Num_Total: 9,
      Max_Dia_mm: 16.6,
      Total_Length_mm: null,       // 缺失就写 null / "None" / ""
      Distortion_pct: 2,
    ...
    },
    {
      ...
      Venue: "Optics Express",
      FoV: "Ultraviolet band 360°×(30°–90°)+ visible band(0-10°)",                 // 字符串 None 也可以
  ...
  const efls  = palData.map(d => d.EFL_mm);

  // ✅ customdata 与字段完全对齐（无 PAL_Type）
  ...
  const layout = {
    ...
    title: {
      text:
        "<span class='cn'>PAL 焦距（</span>" +
        "<span class='non-cn'>EFL</span>" +
        "<span class='cn'>）随年份变化</span>",
      font: { size: 24, color: "#111" },
      x: 0.05
    },
    xaxis: {
    ...
    yaxis: {
      title: { text: "EFL (mm)", font: { family:"Times New Roman", size:16 } },
      tickfont: { family:"Times New Roman" },
      gridcolor: "#e6e6e6"
    },
    hovermode: "closest",
  ...
  function buildCardHTML(pt){
    ...
    return `
      ...
      <div class="row"><span class="cn label">焦距 ${en("EFL")}：</span>${show(pt.y," mm")}</div>
      ...
      <div class="row"><span class="cn label">总片数：</span>${show(d[6])}</div>
