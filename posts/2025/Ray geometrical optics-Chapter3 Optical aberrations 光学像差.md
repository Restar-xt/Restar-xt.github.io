...
特殊而又不失一般性地假设：
- 物点位置位于$x_{o}$轴上，即物点坐标为$(h,0)$

- 物点位置矢量与光线和出瞳面交点地位置矢量的夹角为$\theta$

以上假设虽然很特殊，但是其余一般的情况均是在这种情况下有角度的偏移，可以想象，其他一般的情况即是把坐标系对应旋转某个角度后的特殊情况（让物点落在新的$x_{o}$轴）

对于轴向点物（即当$h=0$时），光学系统绕其光轴的旋转对称性要求像差函数必须具有径向对称性。因此，**像差函数 $W(h;x,y)$ 中所有与$h$无关的项必须随 $x^2+y^2$ 或其整数幂变化。在极坐标下，此类项必然随 $r^2$ 或其整数幂变化。**

>这里像差函数写作$W(h;x,y)$的原因：之前的假设是物点位于$x_{o}$轴上，实际应该写作$W(x_{o},y_{o};x,y)$,但由于$y_{o}=0$，可写作$W(x_{o},0;x,y)$,而$x_{o}=h$，故为了方便写作$W(h;x,y)$，表示的物理意义是：**位于$x_{o}$轴上，高度为$h$的物点发出的光线在出瞳面$(x,y)$处的波像差。**

[![波像差的对称性](https://free.picui.cn/free/2025/09/18/68cbce5845ddb.png)](https://free.picui.cn/free/2025/09/18/68cbce5845ddb.png)

观察上述图片，分析四个点$Q_{1}$、$Q_{2}$、$Q_{3}$、$Q_{4}$对应的波像差之间的大小关系。

首先$x_{o}$轴上发出的，到达出瞳的两条关于$x-z$面对称的光线，波像差应该相同，即：
$$W(h,Q_{1})=W(h,Q_{2})$$
也可以写成：
$$W(h;x,y)=W(h;x,-y)$$
换做极坐标系可写成：
...
从上面两个式子中可以看到，**波像差的展开式一定是$y$的偶次幂，对于极坐标来说，其一定是关于$\cos \theta$的函数。**

另外根据系统的旋转对称性，$Q_{1}$处的波像差和$Q_{3}$处的波像差相等，即：
$$W(h;Q_{1})=W(h;Q_{3})$$
代入直角坐标有：
$$W(h;x,y)=W(-h;-x,-y)$$
换做极坐标系也可以写成：
$$W(h;r,\theta)=W(-h;r,\theta+\pi)$$

从上式可看出，波像差展开式一定是$x^{2}+y^{2}$（$r^{2}$）及其偶次幂、以及$h^{2}$的偶次幂，综上来说，**波像差展开式中不依赖于$\theta$的项必须是$h^{2}r^{2}$或者其整数幂的函数。**
...
$$W(-h;-x,-y)=W(-h;-x,y)$$
换做极坐标可写成：
$$W(-h;r,\theta+\pi)=W(-h;r,-\theta+\pi)$$
而：
$$W(h;r,\theta)=W(-h;r,\theta+\pi)$$
可得：
$$W(h;r,\theta)=W(-h;r,-\theta+\pi)$$

因此，依赖于$\theta$的项必须是$hr \cos \theta$的函数。再与独立于$\theta$的项相结合，可知像差函数由含$h^{2}$、$r^{2}$和$hr \cos \theta$因子的项构成。通常我们将采用像高$h’$而非物高$h$来描述像差函数。
...
显然，在物空间与瞳孔坐标系中，级数的每一项的度数为偶数，且由  $2（l + p + m）$  给出。
